...
public class ParameterValueGetter extends ValueGetter {

    @Override
    public Object getValue(ModelData modelData, String selector) {
        if (modelData == null || modelData.getParameterRepository() == null || StringUtils.isBlank(selector)) {
            return null;
        }

        return modelData.getParameterRepository().findByKey(selector);
    }
}
